...
function creaNuevaDB(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (id,nombre,telefono,email,domicilio,categoria,foto,nota) VALUES (1,'Rin Rin','+6699900970','m.olivarria@ccumazatlan.mx','islote #100 inf playas','pizza','','pin pon es un muñeco')");
}
...
function cargaDatosSuccess(tx, results){
	...
	for(var i=0; i<results.rows.length; i++){
		...
		selector.append('<li id="li_'+persona.id+'"><a href="#detalle" data-uid='+persona.id+' class="linkDetalles"><div class="interior_lista"><img src="'+ foto +'" class="img_peq"/><span>' + persona.nombre + ' ' + persona.telefono+ '</span></div></a><a href="#agregar"  data-theme="a" data-uid='+persona.id+'  class="linkForm">Predet.</a></li>').listview('refresh');
	}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE agenda_curso SET nombre="'+$("#ti_nombre").val()+'",telefono="'+$("#ti_telefono").val()+'",email="'+$("#ti_mail").val()+'",domicilio="'+$("#ti_domicilio").val()+'",categoria="'+cat+'",nota="'+$("#ti_nota").val()+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (nombre,telefono,email,domicilio,categorial,foto,nota) VALUES ('"+$("#ti_nombre").val()+"','"+$("#ti_telefono").val()+"','"+$("#ti_mail").val()+"','"+$("#ti_domicilio").val()+"','"+cat+"','"+$.imageURL+"','"+$("#ti_nota").val()+"')", [], newFormSuccess, errorDB);
}
function newFormSuccess(tx, results) {
	...
	var obj = $('<li id="li_'+results.insertId+'"><a href="#detalle" data-uid='+results.insertId+' class="linkDetalles"><div class="interior_lista"><img src="'+ $.imageUR +'" class="img_peq"/><span>' + $("#ti_nombre").val() + " " + $("#ti_telefono").val()+ '</span></div></a><a href="#agregar"  data-theme="a" data-uid='+results.insertId+'  class="linkForm">Predet.</a></li>');
